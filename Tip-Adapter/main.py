...
def run_tip_adapter_F(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, train_loader_F):
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            cache_logits = ((-1) * (beta - beta * affinity)
                            ).exp() @ cache_values
        ...
        print('LR: {:.6f}, Acc: {:.4f} ({:}/{:}), Loss: {:.4f}'.format(current_lr, correct_samples /
              all_samples, correct_samples, all_samples, sum(loss_list)/len(loss_list)))
        ...
        if acc > best_acc:
            ...
            torch.save(
                adapter.weight, cfg['cache_dir'] + "/best_F_" + str(cfg['shots']) + "shots.pt")
    ...
    # Search Hyperparameters
    best_beta, best_alpha = search_hp(
        cfg, cache_keys, cache_values, val_features, val_labels, clip_weights, adapter=adapter)
...
def main():
    ...
    # CLIP
    if cfg['backbone'] == 'ViT-L/14':
        print(f"\nLoading {cfg['backbone']} using open_clip...")
        # Create model and transforms
        # Note: 'openai' weights are the standard equivalent to original CLIP
        ...
        # Original loading for RN50, ViT-B/32, etc.
        clip_model, preprocess = clip.load(cfg['backbone'])
        clip_model.eval()
        tokenizer = None  # Will use default clip.tokenize in utils
    ...
    val_loader = build_data_loader(
        data_source=dataset.val, batch_size=2, is_train=False, tfm=preprocess, shuffle=False)
    test_loader = build_data_loader(
        data_source=dataset.test, batch_size=2, is_train=False, tfm=preprocess, shuffle=False)

    train_tranform = transforms.Compose([
        transforms.RandomResizedCrop(size=224, scale=(
            0.5, 1), interpolation=transforms.InterpolationMode.BICUBIC),
    ...
    train_loader_cache = build_data_loader(
        data_source=dataset.train_x, batch_size=4, tfm=train_tranform, is_train=True, shuffle=False)
    train_loader_F = build_data_loader(
        data_source=dataset.train_x, batch_size=4, tfm=train_tranform, is_train=True, shuffle=True)

    # Textual features
    print("\nGetting textual features as CLIP's classifier.")
    clip_weights = clip_classifier(
        dataset.classnames, dataset.template, clip_model, tokenizer)
    ...
    # ------------------------------------------ Tip-Adapter ------------------------------------------
    run_tip_adapter(cfg, cache_keys, cache_values, val_features,
