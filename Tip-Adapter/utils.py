...
def clip_classifier(classnames, template, clip_model, tokenizer=None):
    with torch.no_grad():
        ...
        for classname in classnames:
            ...
            # Use custom tokenizer if provided (for OpenCLIP), else use default
...
def build_cache_model(cfg, clip_model, train_loader_cache):
    ...
    if cfg['load_cache'] == False:
        ...
        torch.save(cache_keys, cfg['cache_dir'] +
                   '/keys_' + str(cfg['shots']) + "shots.pt")
        torch.save(cache_values, cfg['cache_dir'] +
                   '/values_' + str(cfg['shots']) + "shots.pt")
    ...
    if cfg['search_hp'] == True:
        ...
        for beta in beta_list:
            for alpha in alpha_list:
                ...
                if acc > best_acc:
                    print("New best setting, beta: {:.2f}, alpha: {:.2f}; accuracy: {:.2f}".format(
                        beta, alpha, acc))
                    best_acc = acc
                    best_beta = beta
                    best_alpha = alpha
